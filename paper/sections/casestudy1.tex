...
\texttt{Linear\allowbreak Discriminant\allowbreak Analysis} by 5-fold stratified cross-validation, so
...
\texttt{FPCLDAClassifier(\allowbreak ncomp=\csOneNComp)}, which performs the FPCA
...
\begin{figure}[tbp]
  ...
  \caption{FPCA scores scatter for the phoneme data (PC1 vs PC2, colour-coded
    ...
    and \textit{sh} are well separated in this projection and \textit{iy}
    largely so, while \textit{aa} and \textit{ao} overlap.  The box reports the
