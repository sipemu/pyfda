...
\end{lstlisting}
    ...
    framed in terms of the supplied task description and domain context.  Provider credentials are entirely the user's
...
\end{lstlisting}
    ...
    default \texttt{guard=True}, a method-specific guard re-checks secondary
    diagnostics after every re-fit---for example, FPCA tuning stops if the
    phase-leakage indicator exceeds 0.5---so the loop cannot improve its target
    metric at the expense of other quality measures.  The loop stops when the
...
language-model provider.  An evaluation suite checks two properties on
constructed data with a known correct answer: comparative method selection
must pick the known-best method, and the tuning loop must move the target
metric in the improving direction and terminate within its budget.  A
...
\ndocpapers{} curated-eligible papers (with DOIs or arXiv identifiers); at
present \ncoverage{} of the \ncallables{} callables (\npubliccallables{} module
functions plus the \texttt{Fdata} methods) have at least one author-verified
paper entry, so coverage is deliberately partial and honest rather than
complete.
