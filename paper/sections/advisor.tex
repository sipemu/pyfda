...
\end{lstlisting}
    ...
    degrees of freedom exceed 0.9 times the number of observed curves in
    the registered dataset (this guard is skipped when that count is
    unavailable), basis-penalty tuning if the optimal
...
\texttt{python/fdars/\_references\_map.json}, that indexes 243 of
the \ncallables{} public callables; 163 of them are linked to at least one
candidate foundational paper from which their algorithms derive, and the
remaining 80 appear only in 10 placeholder records grouping
