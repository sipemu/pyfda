...
requires only one new Rust module and one registration line.
...
\begin{description}
  ...
  \item[Depth, outliers and scoring]
    ...
    \citep{arribas_gil_romo_2014}, MUOD \citep{azcorra_muod_2018}, and
    magnitude-shape (MS) detection \citep{dai_genton_2019}.
  ...
  \item[Clustering and classification]
    ...
    Gaussian mixtures, elastic-alignment clustering, DBSCAN, and FunFEM;
    FPCA-based $k$-NN, LDA and QDA, kernel and DD-classifiers, and an elastic
  ...
  \item[Density, Fr\'{e}chet and metric]
    ...
    \citep{petersen_muller_2019}, and $L^p$/DTW distance matrices.
...
delegate to the binding layer.  Section~\ref{sec:represent} describes it in
full.
...
because their interfaces do not fit the
\texttt{fit}/\texttt{transform}/\texttt{predict} contract --- for example, paired elastic
