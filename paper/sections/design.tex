...
\begin{tikzpicture}[
  ...
  desc/.style={font=\footnotesize\itshape},
  ...
  \node[layer, fill=blue!10] (python) at (0,0)
    {\textbf{Python API layer} (\texttt{fdars})\\[2pt]
     {\desc Fdata container $\cdot$ sklearn estimators $\cdot$ grounded advisor $\cdot$ dataset loaders}};
  \node[layer, fill=orange!10, below=2cm of python] (pyo3)
    {\textbf{PyO3 binding layer} (\texttt{fdars.\_native})\\[2pt]
     {\desc \texttt{\#[pyfunction]} wrappers $\cdot$ \texttt{PyReadonlyArray} views $\cdot$ row\,$\leftrightarrow$\,col-major}};
  \node[layer, fill=green!10, below=2cm of pyo3] (core)
    {\textbf{Rust core} (\texttt{fdars-core})\\[2pt]
     {\desc \texttt{FdMatrix} (nalgebra) $\cdot$ data-parallel loops (rayon) $\cdot$ \nsubmodules{} algorithm families}};
