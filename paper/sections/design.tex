...
\begin{tikzpicture}[
  ...
  \node[sublabel, below=1pt of python]
    {Fdata container $\cdot$ sklearn estimators $\cdot$ grounded advisor $\cdot$ dataset loaders};

  \node[layer, fill=orange!10] (pyo3) at (0,-2.2)
    {\textbf{PyO3 binding layer} (\texttt{fdars.\_native})};
  \node[sublabel, below=1pt of pyo3]
    {\texttt{\#[pyfunction]} wrappers $\cdot$ \texttt{PyReadonlyArray} views $\cdot$ row\,$\leftrightarrow$\,col-major (\texttt{convert.rs})};

  \node[layer, fill=green!10] (core) at (0,-4.4)
    {\textbf{Rust core} (\texttt{fdars-core})};
  \node[sublabel, below=1pt of core]
    {\texttt{FdMatrix} (nalgebra) $\cdot$ data-parallel loops (rayon) $\cdot$ \nsubmodules{} algorithm families};
...
\texttt{rayon}.  \texttt{fdars-core} 0.14.0 is the minimum required version
...
\textbf{PyO3 binding layer.}  A set of \nsubmodules{} Rust modules (one per
...
requires only one new \texttt{\_mod.rs} file and one macro invocation.
...
registry in \texttt{python/fdars/\_\_init\_\_.py} iterates over a fixed list
of submodule names, attaches each to \texttt{sys.modules}, and injects
pure-Python helpers into native submodule namespaces via
\texttt{\_augment.install()}, so both \texttt{fdars.depth.fraiman\_muniz\_1d}
and \texttt{from fdars.depth import fraiman\_muniz\_1d} resolve correctly.
