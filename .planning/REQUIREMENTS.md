...
- [x] **CLUS-01**: `FunctionalKMeans` wrapped as a `ClusterMixin` estimator, deterministic under a fixed `random_state`.
- [x] **CLUS-02**: Differentiator clusterers that pass triage (fuzzy c-means, functional GMM) wrapped as `ClusterMixin` estimators.
...
- [x] **OUT-02**: Newer outlier detectors (tvdmss, muod, depthgram) that pass triage wrapped as `OutlierMixin` estimators, synthesizing a continuous `decision_function` from the underlying method.
...
- [x] **DOCS-03**: Method-accurate hand-authored inline SVG diagram(s) (layer architecture / data flow) meeting the v7.0 STYLE_SPEC + SVGO-idempotence bar; blocking human diagram review before close.
