...
- [x] **INFER-01**: User can run a two-sample integrated-L2 permutation t-test via `fdars.inference.t_perm_test(data_a, data_b, argvals, n_perm=999, seed=None)` and receive a dict `{statistic, p_value, n_perm}`.
- [x] **INFER-02**: User can run a two-sample integrated-F permutation test via `fdars.inference.f_perm_test(...)` returning the same dict shape.
- [x] **INFER-03**: User can run an asymptotic two-sample mean test (Hotelling T² on a shared FPC basis) via `fdars.inference.two_sample_mean_test(data_a, data_b, argvals, ncomp)` → dict (`n_perm=0`, seedless).
...
- [x] **DOCS-06**: Basis/smoothing additions documented (constant_basis + AIC selection) with example(s); advisor `aspects.md` updated to reflect the new `inference` aspect.
...
| INFER-01 | Phase 31 | Complete |
| INFER-02 | Phase 31 | Complete |
| INFER-03 | Phase 31 | Complete |
