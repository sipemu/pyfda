...
- [x] 10-02-PLAN.md — build_diagnostics fpca/basis/smoothing/clustering branches + parameter & method task families (CORE-01, ADVISE-02, ADVISE-03)
...
- [x] 11-01-PLAN.md — TRACER: wire fdars.advisor into public API (__init__.py injection + __all__) + [advisor] pyproject extra + extend test_submodules + first offline test (PYAPI-01, PYAPI-02)

**Wave 2** *(blocked on Wave 1)*

- [x] 11-02-PLAN.md — full offline build_diagnostics tests over docs/data/ + ImportError guard + determinism + env-gated advise integration test (PYAPI-02)
- [x] 11-03-PLAN.md — examples/advisor_recipe.py end-to-end recipe (load → cluster → build_diagnostics → guarded advise) (PYAPI-03)
...
- [x] 12-01-PLAN.md — TRACER: [mcp] extra + HandleRegistry + minimal MCPServer with fdars_build_diagnostics, in-process Client lists+invokes end-to-end (TOOL-01, TOOL-02)
...
**Notes**:
  ...
  - RESOLVED (Phase 12 planning): MCP transport = **stdio only** (CONTEXT locked decision); HTTP/SSE deferred to a future milestone. Tool/handler layer kept transport-agnostic so a future HTTP transport needs no tool rewrite.
  - The `[mcp]` extra requires Python 3.10+ (mcp>=2.0.0); the mcp subpackage guards imports and CI skips MCP tests on Python 3.9.
...
- [x] 13-01-PLAN.md — TRACER: tests/test_skill.py scaffold + offline walkthrough script + first-cut SKILL.md, proven end-to-end (SKILL-01, SKILL-02)

**Wave 2** *(blocked on Wave 1)*

- [x] 13-02-PLAN.md — env-gated advise() grounded-advice step + full SKILL.md body + name==dir/compatibility/py3.9 edge tests (SKILL-01, SKILL-02)
